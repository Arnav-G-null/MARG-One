"""
MARG-One Vision Subsystem: Sign & Gesture Kinematic Interpreter.
Evaluates single-hand and dual-hand skeletal topologies to output structured control values.
"""
...
class SignProcessor:
    """
    Kinematic interpreter for mapping single and dual-hand skeletal configurations
    to discrete values, commands, and telemetry payloads.
    """

    def __init__(self):
        self._custom_evaluators: List[Callable[[List[HandData]], Optional[Dict[str, Any]]]] = []

    def register_custom_sign(self, evaluator_fn: Callable[[List[HandData]], Optional[Dict[str, Any]]]):
        """
        Registers a user-defined evaluation function.
        The function must take `hands: List[HandData]` and return a dictionary or None.
        """
        self._custom_evaluators.append(evaluator_fn)
    ...
    def _get_finger_pattern(self, hand: HandData) -> str:
        """Returns binary 5-character string of digit extension states: T I M R P."""
    ...
    def process_hands(self, hands: List[HandData]) -> Optional[Dict[str, Any]]:
        """
        Evaluates detected hands and returns structured sign metadata and output value.
        """
        if not hands:
            return None

        # 1. Check user-registered custom evaluators first
        for custom_eval in self._custom_evaluators:
            res = custom_eval(hands)
            if res is not None:
                return res
        ...
        if len(hands) >= 2 and left_hand and right_hand:
            ...
            # Two-Hand Sign: Heart Formation
            if index_tip_distance < 65 and thumb_tip_distance < 65:
                return {
            ...
            # Two-Hand Sign: Dual Peace / Victory
